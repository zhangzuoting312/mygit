...
<form class='has-padding content' method='post' action='<?php echo $this->createLink('effort', 'batchCreate');?>' target='hiddenwin' id='batchCreateForm' data-form-refresh='#page'>
  ...
  <div class="control">
    <label for='project'><?php echo $lang->effort->project;?></label>
    <div class='select'>
      <?php echo html::select('project[]', $projects, 0);?>
    </div>
  </div>
  <div class='row'>
    <div class='cell-6'>
...
$(function()
{
    ...
    $(document).on('change', 'select#objectType', function()
    {
        ...
        var $project  = $(this).parent('div').parent('div').next('div.control').find('select#project');
