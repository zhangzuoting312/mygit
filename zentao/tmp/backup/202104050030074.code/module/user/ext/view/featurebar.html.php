<div id="mainMenu" class="clearfix">
  ...
  <?php
    ...
    if($config->global->flow != 'onlyTask' and $config->global->flow != 'onlyTest')
    {
        ...
        common::printLink('user', 'story', "userID={$user->id}", $label, '', "class='btn btn-link $active storyTab'");
    }
    ...
    common::printLink('user', 'dynamic',  "type=today&userID={$user->id}", $label, '', "class='btn btn-link $active dynamicTab'");

    if($config->global->flow == 'full' or $config->global->flow == 'onlyTask')
    {
        $label  = "<span class='text'>{$lang->user->project}</span>";
        $active = $methodName == 'project' ? ' btn-active-text' : '';
        common::printLink('user', 'project',  "userID={$user->id}", $label, '', "class='btn btn-link $active projectTab'");
    }
