...
<div id="mainMenu" class="clearfix">
  <div class="btn-toolbar pull-left">
    <?php echo html::a(inlink('effortcalendar', "projectID=$projectID"), "<span class='text'>{$lang->effort->common}</span>", '', "class='btn btn-link btn-active-text' id='calendarTab'");?>
    <?php if(common::hasPriv('project', 'effort')):?>
    ...
    <div class='input-control space w-150px'>
      <?php echo html::select('account', $users, $userID, "onchange='changeUser($projectID, this.value)' class='form-control chosen'");?>
    </div>
  </div>
  <div class='btn-toolbar pull-right'>
    <?php if(common::hasPriv('effort', 'export')) echo html::a('javascript:exportCalendar("' . helper::createLink('effort', 'export', "userID=$userID&orderBy=date_desc&date=_date_&projectID=$projectID") . '")', "<i class='icon-export muted'></i> " . $lang->effort->export, '', "class='btn btn-link'") ?>
  </div>
</div>
<div class="main-row">
  <div class="main-col">
...
<script>
config.ajaxGetEffortsUrl = '<?php echo $this->createLink('project', 'ajaxGetEfforts', "projectID=$projectID&userID=$userID&year={year}");?>';
...
function changeUser(projectID, userID)
{
    location.href = createLink('project', 'effortcalendar', 'projectID=' + projectID + '&userID=' + userID); 
}
</script>
