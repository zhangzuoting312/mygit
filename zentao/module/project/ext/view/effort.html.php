...
<div id="mainMenu" class="clearfix">
  <div class="btn-toolbar pull-left">
    <?php echo common::hasPriv('project', 'effortcalendar') ? html::a(helper::createLink('project', 'effortcalendar', "prejectID=$projectID"), $lang->effort->common, '', "class='btn btn-link' id='calendarTab'") : '';?>
    ...
    <div class='input-control space w-150px'>
      <?php echo html::select('account', $users, $userID, "class='form-control chosen' id='account' onchange='changeUser($projectID, \"$date\", this.value)'");?>
    </div>
  </div>
...
<div id="mainContent">
  <form class='main-table' data-ride='table' id='effortForm' method='post' action='<?php echo $this->createLink('effort', 'batchEdit', "from=browse&userID=" . ($userID == 'all' ? '' : $userID))?>'>
    <table id='effortList' class='table has-sort-head table-fixed'>
      <?php $vars = "projectID=$projectID&date=$date&userID=$userID&orderBy=%s&recTotal=$pager->recTotal&recPerPage=$pager->recPerPage"; ?>
      <thead>
        <tr>
