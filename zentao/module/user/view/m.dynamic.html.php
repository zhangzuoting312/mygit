...
<nav id='subMenu' class='menu nav gray'>
  <?php 
  echo html::a(inLink('dynamic', "period=today&userID={$user->id}"),      $lang->action->dynamic->today);
  echo html::a(inLink('dynamic', "period=yesterday&userID={$user->id}"),  $lang->action->dynamic->yesterday);
  echo html::a(inLink('dynamic', "period=twodaysago&userID={$user->id}"), $lang->action->dynamic->twoDaysAgo);
  echo html::a(inLink('dynamic', "period=thisweek&userID={$user->id}"),   $lang->action->dynamic->thisWeek);
  echo html::a(inLink('dynamic', "period=lastweek&userID={$user->id}"),   $lang->action->dynamic->lastWeek);
  echo html::a(inLink('dynamic', "period=thismonth&userID={$user->id}"),  $lang->action->dynamic->thisMonth);
  echo html::a(inLink('dynamic', "period=lastmonth&userID={$user->id}"),  $lang->action->dynamic->lastMonth);
  echo html::a(inLink('dynamic', "period=all&userID={$user->id}"),        $lang->action->dynamic->all);
  ?>
